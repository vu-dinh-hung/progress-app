...
@pytest.mark.parametrize('user_data, invalid_field', [
    ...
    pytest.param({'username': 'user_with_missing_password'}, 'password', id='missing password')
])
...
@pytest.mark.parametrize('user_data', [
    pytest.param({'username': 'userfromtest', 'password': 'p4ssw0rd'}, id='without name'),
    pytest.param({'username': 'userfromtest1', 'password': 'p4ssw0rd', 'name': 'Test'}, id='with name')
])
...
@pytest.mark.parametrize('user_data, invalid_field', [
    pytest.param({'password': 'p4ssw0rd'}, 'username', id='missing username'),
    pytest.param({'username': 'user_with_missing_password'}, 'password', id='missing password'),
    pytest.param({'username': 'user_with_short_password', 'password': 'short'}, 'password', id='short password'),
    pytest.param({'username': 'user_with_white_space_password', 'password': 'white space'}, 'password', id='white space password')
])
def test_post_user_errors(client, db_populated, users_in_db_getter, user_data, invalid_field):
...
def test_post_user_errors_duplicate_username(client, db_populated, users_in_db_getter):
    ...
    res = client.post('/api/users', data=json.dumps({
        'username': users_before[0].username,
        'password': 'p4ssw0rd'
    }))
...
def test_get_user(client, db_populated, users_in_db_getter, jwt_user_0):
    ...
    res = client.get(f'/api/users/{users[0].id}', headers={'Authorization': f'Bearer {jwt_user_0}'})
