...
def test_get_habits(client, db_populated, users_in_db_getter, habits_in_db_getter, jwt_user_0):
    users = users_in_db_getter()

    logyear = 2021
    logmonth = 6

    res1 = client.get(f'/api/users/{users[0].id}/habits?page=1&{logyear=}&{logmonth=}', headers={'Authorization': f'Bearer {jwt_user_0}'})
    res2 = client.get(f'/api/users/{users[0].id}/habits?page=2&{logyear=}&{logmonth=}', headers={'Authorization': f'Bearer {jwt_user_0}'})
...
def test_get_habits_errors(client, db_populated, users_in_db_getter, jwt_user_0):
    ...
    missing_queries = client.get(f'/api/users/{users[0].id}/habits', headers={'Authorization': f'Bearer {jwt_user_0}'})
    missing_auth = client.get(f'/api/users/{users[0].id}/habits?page=2&{logyear=}&{logmonth=}')
    wrong_user = client.get(f'/api/users/{users[1].id}/habits?page=2&{logyear=}&{logmonth=}', headers={'Authorization': f'Bearer {jwt_user_0}'})
